...
-- find the department name with height avg salary 
SELECT department_name, round(avg(salary)) as avg_salary FROM employees as e
    JOIN departments USING(department_id)
    GROUP BY department_name
...
SELECT extract(YEAR from hire_date) as year, count(*) as employees FROM employees
    JOIN departments USING(department_id)
    GROUP BY year
...
SELECT customer_id, count(*) as orders, sum(total_amount) as total_spend from orders
 GROUP BY customer_id
 HAVING count(customer_id) > 2;

-- Find the total amount of orders placed each month in the year 2022.

SELECT extract(month from order_date) as order_month, sum(total_amount) as total FROM orders
